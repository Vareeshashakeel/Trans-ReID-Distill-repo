...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    # Minimal local implementation for repo self-containment
    def norm_cdf(x):
...
def resize_pos_embed(posemb, posemb_new, hight, width):
    ...
    print(f'Resized position embedding from size:{posemb_new.shape if False else posemb.shape} to size: {posemb.shape} with height:{hight} width: {width}')
    return posemb
...
class TransReID(nn.Module):
    """Backbone that can optionally use camera embeddings for teacher training/inference."""

    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.0, qkv_bias=False, qk_scale=None,
                 drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.0, norm_layer=nn.LayerNorm,
                 camera_num=0, use_camera_embed=False):
        super().__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim
        self.use_camera_embed = use_camera_embed and camera_num is not None and camera_num > 0
        self.camera_num = camera_num if camera_num is not None else 0
        ...
        if self.use_camera_embed:
            self.sie_embed = nn.Parameter(torch.zeros(self.camera_num, 1, embed_dim))
    ...
    def no_weight_decay(self):
        if self.use_camera_embed:
            return {'pos_embed', 'cls_token', 'sie_embed'}
        return {'pos_embed', 'cls_token'}

    def forward_features(self, x, cam_label=None, return_intermediate=False, out_indices=None):
        batch_size = x.shape[0]
        x = self.patch_embed(x)
        cls_tokens = self.cls_token.expand(batch_size, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        x = x + self.pos_embed
        if self.use_camera_embed and cam_label is not None:
            cam_label = cam_label.long().clamp_(min=0, max=self.camera_num - 1)
            x = x + self.sie_embed[cam_label]
        x = self.pos_drop(x)
    ...
    def forward(self, x, cam_label=None, return_intermediate=False, out_indices=None):
        return self.forward_features(x, cam_label=cam_label, return_intermediate=return_intermediate, out_indices=out_indices)
    ...
    def load_param(self, model_path, load=False):
        ...
        for key, value in param_dict.items():
            ...
            try:
                ...
                # Allow loading student pretrain into teacher/student even when camera embedding differs
                if key in self.state_dict():
                    print('skip unmatched key:', key, value.shape, self.state_dict()[key].shape)
